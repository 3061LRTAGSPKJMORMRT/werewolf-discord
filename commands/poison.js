  ...
  run: async (message, args, client) => {
    if (message.channel.name == "priv-witch") {
      ...
      if (night == 1) {
        ...
        let ability = await db.fetch(`witch_${message.channel.id}`);
        if (ability == 1) {
              ...
              if (!guy.roles.cache.has(alive.id) || !ownself.roles.cache.has(alive.id)) {
                ...
                if (args[0] == message.member.nickname) {
                  ...
                  if (isNight !== "yes") {
                    ...
                    dayChat.send(
                      `The Witch poisoned **${args[0]} ${guy.username} (${role})**!`
                    );
                    guy.addRole(dead.id)
                    guy.removeRole(alive.id)
                    if (role == 'Junior Werewolf') {
                      let tag = await db.fetch(`jwwtag_${guy.id}`)
                      if (tag != null) {
                        let killed = message.guild.members.cache.find(m => m.nickname === tag) 
                        let newRole = await db.fetch(`role_${killed.id}`) 
                        killed.removeRole(alive.id)
                        killed.addRole(dead.id)
                        dayChat.send(`The Junior Werewolf's death has been avenged! **${tag} ${killed.username} (${killed})** is dead!`)
                      } 
                    } 
